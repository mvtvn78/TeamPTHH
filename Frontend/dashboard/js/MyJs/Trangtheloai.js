...
function show()
{
    queryDataGet("http://localhost:8001/api/v1/genre","",function(res){
        // lấy dữ liệu từ api
        const data = res.data
        // lấy error code để check
        const code = res.ErrorCode
...
function showSearch ()
{
    ...
    queryDataGet("http://localhost:8001/api/v1/genre_search",datasend,function(res){
        console.log(res);
        // lấy dữ liệu từ api
        const data = res.data
        // lấy error code để check
...
$(".nguoidung_tb").on('click','.nut_xoa',function()
{
  ...
  bootbox.confirm('Bạn có chắc xóa '+ma+' mã  này ?',function(result)
  {
    if (result==true)
    {
        queryDataDelete("http://localhost:8001/api/v1/genre_remove",{maTL:ma},function(res)
        {
            if(res.ErrorCode ==0)
...
$(".btnsave").click(function()
{
    ...
    if(mode ==1)
    {
        ...
        console.log(datasend);
        queryDataPost("http://localhost:8001/api/v1/genre_add",datasend,function(res)
        {
            if(res.ErrorCode ==0)
            {
