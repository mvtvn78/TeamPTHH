...
function show()
{
    queryDataGet("http://localhost:6969/api/v1/followers","",function(res){
        // lấy dữ liệu từ api
        const data = res.data
        ...
        else 
        {
            ...
            for(const item in data){
                ...
                const hienthingaytheodoi = ConvertDate(ngaytheodoi.toLocaleDateString())
                        htmls=htmls+'<tr>'+
                                '<td>'+obj.MAND+'</td>'+
                                '<td>'+obj.MANS+'</td>'+
...
function showSearch ()
{
    ...
    console.log(keyseacrh);
        queryDataGet(`http://localhost:6969/api/v1/followers_search?maND=${keyseacrh}&maNS=${keyseacrh}`,'',function(res){
            // lấy dữ liệu từ api
        const data = res.data
        // lấy error code để check
        const code = res.ErrorCode
...
$(".theodoi_tb").on('click','.nut_xoa',function()
{
  ...
  bootbox.confirm('Bạn có chắc xóa ' +maND+ ' và ' +maNS+ ' mã  này ?',function(result)
  {
    if (result==true)
    {
        queryDataDelete("http://localhost:6969/api/v1/followers_remove",{maND:maND,maNS:maNS},function(res)
        {
            if(res.ErrorCode ==0)
...
$(".btnsave").click(function()
{
    ...
    if(mode ==1)
    {
        ...
        queryDataPost("http://localhost:6969/api/v1/followers_add",datasend,function(res)
        {
            if(res.ErrorCode ==0)
            {
